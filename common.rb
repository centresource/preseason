require File.join(File.dirname(__FILE__), 'core_extensions.rb')

# create/use gemset
require 'rvm'
run "rvm gemset create #{app_name}"
RVM.gemset_use! app_name # `run "rvm gemset use #{app_name}"` doesn't work -- rvm still uses the terminal's current gemset
create_file '.rvmrc', "rvm gemset use #{app_name}"
run 'rvm rvmrc trust'

initialize_template

load_options

#loop through the recipes and apply each one
required_recipes = %w(database gitignore gemfile production bundle routes flash_messages application schedule rspec whiskey_disk foreman active_admin authentication guard spork_rspec git)
required_recipes.each {|required_recipe| apply recipe(required_recipe)}

